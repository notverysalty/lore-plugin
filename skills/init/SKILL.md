...
2. **Create the directory and first files** (`docs/ai-knowledge/` is guarded by a PreToolUse gate; invoking this skill auto-issued the grant for this turn — no manual step needed):
   ...
   - `docs/ai-knowledge/lore.json` — `{"language": "<tag>"}`; ask the user which language knowledge should be written in (BCP 47 tag; default `en`). Optional keys the team can add later: `promotionTarget` (where cross-repo knowledge is promoted — see knowledge-consolidate), `indexMode` / `indexGroupThreshold` (index scaling). Write the contract/gotchas bodies below — and all seeded entries — in that language (frontmatter keys and enum values stay English).
